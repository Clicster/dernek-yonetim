              ...
              <tr className="text-gray-500 text-left border-b border-gray-800">
                ...
                {periyotlar.map((p) => (
                  <th key={`c-${periyotKey(p)}`} className="px-3 py-2 text-center whitespace-nowrap text-xs">
                    {periyotKey(p)}<br /><span className="text-green-500">Çalışma</span>
                  </th>
                ))}
              </tr>
            </thead>
              ...
              {kisiler.map((k) => {
                ...
                return (
                  <tr key={k.id} className="border-b border-gray-800/50 hover:bg-gray-800/30">
                    ...
                    {periyotlar.map((p) => (
                      <td key={`c-${periyotKey(p)}`} className="px-3 py-2 text-center text-green-400 font-mono text-xs tabular-nums whitespace-nowrap">
