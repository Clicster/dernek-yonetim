...
import {
  ...
  Ekstra, Ikramiye, KurFiyatlari, Period, PERIODLAR, SureKisi,
  UYELIK_TURLERI, Uyelik, UyelikTuru, Yonetici, gundenPeriod,
} from "@/lib/types";
import { useAy } from "@/lib/ay-context";

type MainTab = "dernek" | "yonetim" | "konsey";
type DernekSection = "uyelikler" | "ikramiyeler" | "ekstralar" | "coinler" | "yoneticiler" | "ayarlar";
...
// Tüm eşleşen kişilerde süreyi senkronize et
function syncHabboismi(data: AppData, habboismi: string, aylar: Record<Ay, AylikVeri>): AppData {
  if (!habboismi) return data;
  const updated = { ...data };
  updated.chd = { ...updated.chd, yoneticiler: updated.chd.yoneticiler.map((y) => y.habboismi === habboismi ? { ...y, aylar } : y) };
  updated.treachery = { ...updated.treachery, yoneticiler: updated.treachery.yoneticiler.map((y) => y.habboismi === habboismi ? { ...y, aylar } : y) };
...
export default function AdminDashboard() {
  ...
  const [mainTab, setMainTab] = useState<MainTab>("dernek");
  const [activeTab, setActiveTab] = useState<DernekKey>("chd");
  const [activeSection, setActiveSection] = useState<DernekSection>("uyelikler");
  ...
  const removeUyelik = async (id: string) => {
    const u = { ...data };
    u[activeTab].uyelikler = u[activeTab].uyelikler.filter((x) => x.id !== id);
    u[activeTab].coinHarcamalar = u[activeTab].coinHarcamalar.filter((x) => x.id !== id + "c");
    await save(u);
  };
  const removeIkramiye = async (id: string) => {
    const u = { ...data };
    u[activeTab].ikramiyeler = u[activeTab].ikramiyeler.filter((x) => x.id !== id);
    u[activeTab].coinHarcamalar = u[activeTab].coinHarcamalar.filter((x) => x.id !== id + "c");
    await save(u);
  };
  const removeCoin = async (id: string) => {
    const u = { ...data };
    u[activeTab].coinHarcamalar = u[activeTab].coinHarcamalar.filter((x) => x.id !== id);
    await save(u);
  };
  const removeEkstra = async (id: string) => {
    const u = { ...data };
    u[activeTab].ekstralar = (u[activeTab].ekstralar ?? []).filter((x) => x.id !== id);
    u[activeTab].coinHarcamalar = u[activeTab].coinHarcamalar.filter((x) => x.id !== id + "c");
    await save(u);
  };
  const removeYonetici = async (id: string) => {
    const u = { ...data };
    u[activeTab].yoneticiler = u[activeTab].yoneticiler.filter((x) => x.id !== id);
    await save(u);
  };
        ...
        {(["dernek", "yonetim", "konsey"] as MainTab[]).map((t) => (
          <button key={t} onClick={() => setMainTab(t)}
            className={`px-5 py-2 rounded-lg font-semibold text-sm transition-colors ${
              mainTab === t
                ? t === "dernek" ? "bg-indigo-600 text-white" : t === "yonetim" ? "bg-emerald-600 text-white" : "bg-purple-600 text-white"
                : "bg-gray-800 text-gray-400 hover:text-white"
            }`}>
            {t === "dernek" ? "Dernek" : t === "yonetim" ? "Yönetim Süre" : "Konsey Süre"}
          </button>
        ))}
      </div>
      ...
      {mainTab === "dernek" && (
        <>
          ...
          <div className="flex gap-2 border-b border-gray-800 pb-2 flex-wrap">
            {(["uyelikler", "ikramiyeler", "ekstralar", "coinler", "yoneticiler", "ayarlar"] as DernekSection[]).map((s) => (
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead><tr className="text-gray-500 text-left border-b border-gray-800">
                    <th className="px-4 py-2">Tarih</th><th className="px-4 py-2">Tür</th>
                    <th className="px-4 py-2 text-center">Adet</th><th className="px-4 py-2 text-right">Kur</th>
                    <th className="px-4 py-2 text-right">Coin</th><th className="px-4 py-2"></th>
                  </tr></thead>
                  <tbody>
                    {uyelikler.map((u) => (
            ...
            {activeSection === "ikramiyeler" && (
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead><tr className="text-gray-500 text-left border-b border-gray-800">
                    <th className="px-4 py-2">Tarih</th><th className="px-4 py-2">İsim</th>
                    <th className="px-4 py-2 text-center">Kredi</th><th className="px-4 py-2 text-center">Çarpan</th>
                    <th className="px-4 py-2">Yönetici</th><th className="px-4 py-2 text-right">Coin</th><th className="px-4 py-2"></th>
                  </tr></thead>
                  <tbody>
                    {ikramiyeler.map((i) => (
            ...
            {activeSection === "ekstralar" && (
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead><tr className="text-gray-500 text-left border-b border-gray-800">
                    <th className="px-4 py-2">Tarih</th><th className="px-4 py-2">Açıklama</th>
                    <th className="px-4 py-2 text-right">Coin</th><th className="px-4 py-2"></th>
                  </tr></thead>
                  <tbody>
                    {ekstralar.map((e) => (
            ...
            {activeSection === "coinler" && (
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead><tr className="text-gray-500 text-left border-b border-gray-800">
                    <th className="px-4 py-2">Tarih</th><th className="px-4 py-2 text-right">Miktar</th>
                    <th className="px-4 py-2">Kategori</th><th className="px-4 py-2">Açıklama</th><th className="px-4 py-2"></th>
                  </tr></thead>
                  <tbody>
                    {coinler.map((c) => (
            ...
            {activeSection === "ayarlar" && (
              <KurFiyatlariAyar kur={kur} onSave={async (yeniKur) => {
                const u = { ...data };
          ...
          {activeSection === "yoneticiler" && (
            <div className="flex justify-end -mt-4">
              <button onClick={() => { setEditYonetici(null); setModal("yonetici-edit"); }}
          ...
          {/* Dernek Modalleri */}
          {modal === "uyelik" && (
            <UyelikModal kur={kur} yoneticiler={d.yoneticiler} onClose={() => setModal(null)}
              onSave={async (uyelik, coinEntry) => {
                const u = { ...data };
                u[activeTab].uyelikler.push(uyelik);
                if (coinEntry) u[activeTab].coinHarcamalar.push(coinEntry);
                await save(u); setModal(null);
              }} />
          )}
          {modal === "ikramiye" && (
            <IkramiyeModal yoneticiler={d.yoneticiler} onClose={() => setModal(null)}
          ...
          {modal === "ekstra" && (
            <EkstraModal onClose={() => setModal(null)}
          ...
          {modal === "yonetici-edit" && (
            editYonetici ? (
              <YoneticiDuzenleModal yonetici={editYonetici}
                onClose={() => { setModal(null); setEditYonetici(null); }}
                onSave={async (item) => {
                  const u = { ...data };
                  u[activeTab].yoneticiler = u[activeTab].yoneticiler.map((y) => y.id === item.id ? item : y);
                  await save(u); setModal(null); setEditYonetici(null);
                }} />
            ) : (
              <YoneticiEkleModal onClose={() => setModal(null)}
                onSave={async (item) => {
                  const u = { ...data };
          ...
          {modal === "sure-gir" && sureHedefYonetici && (
            <SureGirModal yonetici={sureHedefYonetici} varsayilanAy={secilenAy}
              onClose={() => { setModal(null); setSureHedefYonetici(null); }}
              onSave={async (guncelY) => {
                let u = { ...data };
                u[activeTab].yoneticiler = u[activeTab].yoneticiler.map((y) => y.id === guncelY.id ? guncelY : y);
                // Senkronizasyon
                if (guncelY.habboismi) u = syncHabboismi(u, guncelY.habboismi, guncelY.aylar);
                await save(u); setModal(null); setSureHedefYonetici(null);
              }} />
          )}
        </>
      )}
    ...
    <div>
      ...
      <div className="flex items-center gap-2">
        ...
        <input type="number" min={1} max={31} value={gun}
          ...
          className="w-20 bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-indigo-500 text-center" placeholder="Gün" />
      </div>
    </div>
  );
}
...
function UyelikModal({ kur, yoneticiler, onClose, onSave }: { kur: KurFiyatlari; yoneticiler: Yonetici[]; onClose: () => void; onSave: (u: Uyelik, coin: CoinHarcama | null) => void }) {
  ...
  const handleSave = () => {
    ...
    const coinEntry: CoinHarcama | null = coinMiktar > 0 ? { id: id + "c", tarih, miktar: coinMiktar, kategori: "Üyelik", aciklama: `${adet}x ${tur} üyelik` } : null;
        ...
        <div>
          ...
          <div className="grid grid-cols-4 gap-2">
            {UYELIK_TURLERI.map((t) => (
              <button key={t} onClick={() => setTur(t)} className={`py-2 rounded-lg text-sm font-bold transition-colors ${tur === t ? "bg-indigo-600 text-white" : "bg-gray-800 text-gray-400 hover:text-white"}`}>{t}</button>
            ))}
          </div>
        </div>
        <div>
          <label className="block text-gray-400 text-sm mb-1">Adet</label>
          <input type="number" min={1} value={adet} onChange={(e) => setAdet(Math.max(1, Number(e.target.value)))}
            className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-indigo-500" />
        </div>
        <div className="bg-gray-800/60 rounded-lg px-4 py-3 flex items-center justify-between">
          <div className="text-gray-400 text-sm"><span>{adet} × {kurFiyati} (kur)</span></div>
          <span className="text-yellow-400 font-bold text-lg">{coinMiktar} Coin</span>
        </div>
        {kurFiyati === 0 && <p className="text-amber-500 text-xs">Bu tür için kur fiyatı 0. Ayarlar sekmesinden güncelleyin.</p>}
...
function IkramiyeModal({ yoneticiler, onClose, onSave }: { yoneticiler: Yonetici[]; onClose: () => void; onSave: (i: Ikramiye, coin: CoinHarcama | null) => void }) {
  const { ayIndex, yil } = useAy();
  const [gun, setGun] = useState<number>(1);
  ...
  const handleSave = () => {
    ...
    const coinEntry: CoinHarcama | null = coinMiktar > 0 ? { id: id + "c", tarih, miktar: coinMiktar, kategori: "İkramiye", aciklama: `${isim} — ${carpan}x ${kredi} kredi (${yonetici})` } : null;
    ...
    <ModalWrapper title="İkramiye Ekle" onClose={onClose}>
      <div className="space-y-4">
        ...
        <div>
          ...
          <input value={isim} onChange={(e) => setIsim(e.target.value)} className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-indigo-500" placeholder="İkramiye adı" />
        </div>
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="block text-gray-400 text-sm mb-1">Kredi</label>
            <input type="number" min={0} value={kredi} onChange={(e) => setKredi(Math.max(0, Number(e.target.value)))} className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-indigo-500" />
          </div>
          <div>
            <label className="block text-gray-400 text-sm mb-1">Çarpan (x)</label>
            <input type="number" min={1} value={carpan} onChange={(e) => setCarpan(Math.max(1, Number(e.target.value)))} className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-indigo-500" />
          </div>
        </div>
        <div>
          <label className="block text-gray-400 text-sm mb-1">Yönetici</label>
          {yoneticiler.length > 0 ? (
            <select value={yonetici} onChange={(e) => setYonetici(e.target.value)} className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-indigo-500">
              {yoneticiler.map((y) => <option key={y.id} value={y.ad}>{y.ad} — {y.rol}</option>)}
            </select>
          ) : (
            <input value={yonetici} onChange={(e) => setYonetici(e.target.value)} className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-indigo-500" placeholder="Yönetici adı" />
          )}
        </div>
        <div className="bg-gray-800/60 rounded-lg px-4 py-3 flex items-center justify-between">
        ...
        <div className="flex gap-3 pt-1">
          <button onClick={onClose} className="flex-1 py-2 rounded-lg border border-gray-700 text-gray-400 hover:text-white text-sm transition-colors">İptal</button>
          <button onClick={handleSave} disabled={!isim.trim()} className="flex-1 py-2 rounded-lg bg-indigo-600 hover:bg-indigo-700 disabled:opacity-40 text-white font-medium text-sm transition-colors">Kaydet</button>
        </div>
      </div>
    </ModalWrapper>
  );
...
function KurFiyatlariAyar({ kur, onSave }: { kur: KurFiyatlari; onSave: (k: KurFiyatlari) => void }) {
  ...
  return (
    <div className="p-4 space-y-4">
      <p className="text-gray-400 text-sm">Her üyelik türü için coin kur fiyatını belirle.</p>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {UYELIK_TURLERI.map((t) => (
          <div key={t}>
            <label className="block text-gray-400 text-sm mb-1">{t} (Coin)</label>
            <input type="number" min={0} value={vals[t]} onChange={(e) => set(t, Math.max(0, Number(e.target.value)))}
      ...
      <button onClick={() => onSave(vals)} className="px-6 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-sm font-medium transition-colors">Kaydet</button>
    </div>
  );
}
...
function YoneticiEkleModal({ onClose, onSave }: { onClose: () => void; onSave: (y: Yonetici) => void }) {
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    onSave({ id: Date.now().toString() + Math.random().toString(36).slice(2), ad: f.get("ad") as string, rol: f.get("rol") as string, habboismi: (f.get("habboismi") as string) || undefined, aylar: emptyAylar() });
  };
  return (
    <ModalWrapper title="Yönetici Ekle" onClose={onClose}>
      <form onSubmit={handleSubmit} className="space-y-4">
        <InputField label="Ad Soyad" name="ad" required />
        <InputField label="Habbo Kullanıcı Adı (opsiyonel)" name="habboismi" />
        <div>
...
function YoneticiDuzenleModal({ yonetici, onClose, onSave }: { yonetici: Yonetici; onClose: () => void; onSave: (y: Yonetici) => void }) {
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    const f = new FormData(e.currentTarget);
    onSave({ ...yonetici, ad: f.get("ad") as string, rol: f.get("rol") as string, habboismi: (f.get("habboismi") as string) || undefined });
  };
  return (
    <ModalWrapper title="Yönetici Düzenle" onClose={onClose}>
      <form onSubmit={handleSubmit} className="space-y-4">
        <InputField label="Ad Soyad" name="ad" required defaultValue={yonetici.ad} />
        <InputField label="Habbo Kullanıcı Adı (opsiyonel)" name="habboismi" defaultValue={yonetici.habboismi} />
        <div>
          <label className="block text-gray-400 text-sm mb-1">Rol</label>
...
// ─── Süre Giriş Modalı (Yonetici) ─────────────────────────────────────────────

function SureGirModal({ yonetici, varsayilanAy, onClose, onSave }: { yonetici: Yonetici; varsayilanAy: Ay; onClose: () => void; onSave: (y: Yonetici) => void }) {
  ...
  const handleSave = () => {
    ...
    if (!guncelYonetici.aylar[ay]) guncelYonetici.aylar[ay] = { oda: Object.fromEntries(PERIODLAR.map((p) => [p, 0])) as Record<Period, number>, calisma: Object.fromEntries(PERIODLAR.map((p) => [p, 0])) as Record<Period, number> };
...
function EkstraModal({ onClose, onSave }: { onClose: () => void; onSave: (e: Ekstra, coin: CoinHarcama | null) => void }) {
  const { ayIndex, yil } = useAy();
  const [gun, setGun] = useState<number>(1);
  ...
  const handleSave = () => {
    ...
    const coinEntry: CoinHarcama = { id: id + "c", tarih, miktar: coinMiktar, kategori: "Ekstra", aciklama };
    ...
    <ModalWrapper title="Ekstra Ekle" onClose={onClose}>
      <div className="space-y-4">
        ...
        <div>
          ...
          <input value={aciklama} onChange={(e) => setAciklama(e.target.value)} className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-indigo-500" placeholder="Açıklama yazın..." />
        </div>
        <div>
          <label className="block text-gray-400 text-sm mb-1">Coin Miktarı</label>
          <input type="number" min={0} value={coinMiktar} onChange={(e) => setCoinMiktar(Math.max(0, Number(e.target.value)))} className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-indigo-500 text-center font-bold" />
        </div>
        <div className="bg-gray-800/60 rounded-lg px-4 py-3 flex items-center justify-between">
        ...
        <div className="flex gap-3 pt-1">
          <button onClick={onClose} className="flex-1 py-2 rounded-lg border border-gray-700 text-gray-400 hover:text-white text-sm transition-colors">İptal</button>
          <button onClick={handleSave} disabled={!aciklama.trim() || coinMiktar <= 0} className="flex-1 py-2 rounded-lg bg-indigo-600 hover:bg-indigo-700 disabled:opacity-40 text-white font-medium text-sm transition-colors">Kaydet</button>
        </div>
      </div>
    </ModalWrapper>
  );
